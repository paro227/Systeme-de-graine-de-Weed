    ...
    ESX.RegisterUsableItem(item_name, function(source)
        ...
        if lastTime and currentTime - lastTime < 10 then
            TriggerClientEvent("pNotify:SendNotification", source, {
                text = 'Attendre au moins 10 secondes avant de recommencer cette opération.',
                type = "error",
                timeout = 2000,
                layout = "centerLeft"
            })
        ...
        if cops >= options[item_name].cops then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Pour démarrer cette opération, il doit y avoir au moins '.. options[item_name].cops .. ' policiers en service.',
                type = "error",
                timeout = 5000,
                layout = "centerLeft"
            })
            TriggerClientEvent("pNotify:SendNotification", _source, {
                text = 'Le nombre actuel de policiers en service: '.. cops ..'/'.. options[item_name].cops,
                type = "info",
                timeout = 6000,
                layout = "centerLeft"
            })
...
AddEventHandler("esx_receptury:statusSuccess", function(message, min, max, item)
    ...
    if(itemProps.limit < itemProps.count + amount) then
        ...
        TriggerClientEvent("pNotify:SendNotification", source, {
            text = 'Vous abandonnez une partie de la marchandise car vous n\'avez plus de place dans vos poches.',
            type = "error",
            timeout = 2000,
            layout = "centerLeft"
        })
